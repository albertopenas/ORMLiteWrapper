package es.sw.activerecordapp.oldddbb;
...
import es.sw.activerecordapp.DatabaseHelper;
...
public abstract class ActiveRecord extends Object{
    ...
    //TODO: hacer los metodos de RX
    //TODO: hacer versiones sin capturar excepciones y versiones con excepciones, los primeros son los de RX, los otros con try....
    public static<T extends ActiveRecord> Observable<Boolean> saveRX(Context context, T t) {
        return makeObservable(new Callable<Boolean>() {
            @Override
            public Boolean call() throws Exception {
                return save(context, t);
            }
        });
        //return createObservable(() -> save(context,t));
    }

    //TODO: maybe not static needed
    public static<T extends ActiveRecord> Observable<Boolean> saveWithExceptionRX() {
        return makeObservable(new Callable<Boolean>() {
            @Override
            public Boolean call() throws Exception {
                throw new Exception("fallo a posta");
            }
        });
    }
